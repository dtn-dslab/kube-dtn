...
const (
	...
	defaultCNIFile    = "00-kubedtn.conflist"
	interNodeLinkConf = "/etc/cni/net.d/kubedtn-inter-node-link-type"
	defaultPluginName = "kubedtn"
)

var kubedtnCNIPath = filepath.Join(defaultNetDir, defaultCNIFile)
...
func loadConfList() (map[string]interface{}, error) {
	...
	// Ignore any existing kubedtn config files
	var confFiles []string
	for _, f := range files {
		if strings.Contains(f, "kubedtn") {
			continue
		}
...
func saveConfList(m map[string]interface{}) error {
	...
	return ioutil.WriteFile(kubedtnCNIPath, bytes, os.FileMode(06444))
}
...
// Init installs kubedtn CNI configuration
func Init() error {
...
// Cleanup removes kubedtn CNI configuration
func Cleanup() {
	if err := os.Remove(kubedtnCNIPath); err != nil {
		log.Infof("Failed to remove file %s: %v", kubedtnCNIPath, err)
	}
