...
var grpcLogger *log.Entry = nil

func InitLogger() {
	grpcLogger = log.WithFields(log.Fields{"daemon": "kubedtnd", "overlay": "gRPC"})
}
...
func DeleteWiresByPod(namespace string, podName string) error {
	...
	if !ok || len(wires) == 0 {
		grpcLogger.Infof("[WIRE-DELETE]:All grpc-wires for pod %s:%s is already deleted", namespace, podName)
		return nil
	}
	grpcLogger.Infof("[WIRE-DELETE]:Removing %d grpc-wires for pod %s@%s", len(wires), podName, namespace)
	var errs errlist.List
	for _, w := range wires {
		grpcLogger.Infof("[WIRE-DELETE]:Removing local-iface@pod : %s@%s for wire UID: %d", w.LocalPodIfaceName, w.LocalPodName, w.UID)
		if err := RemoveWire(w); err != nil {
			grpcLogger.Infof("[WIRE-DELETE]:Error Removing local-iface@pod : %s@%s for wire UID: %d : %v", w.LocalPodIfaceName, w.LocalPodName, w.UID, err)
			errs.Add(err)
		}
	}
	if errs.Err() != nil {
	...
	grpcLogger.Infof("[WIRE-DELETE]:Successfully removed all grpc-wires for pod %s@%s", podName, namespace)
	return nil
}
...
func RemoveWire(wire *GRPCWire) error {
	if wire == nil {
		grpcLogger.Infof("[WIRE-DELETE]:Null wire. This ware is already removed")
		return nil
	}
	...
	if err != nil {
		grpcLogger.Infof("[WIRE-DELETE]:Interface index %d for wire %d, is already cleaned up.", wire.LocalNodeIfaceID, wire.UID)
	} else {
		myVeth := koko.VEth{}
	...
	DeleteWire(wire)
	grpcLogger.Infof("[WIRE-DELETE]:Successfully removed grpc wire for link %d.", wire.UID)
	return nil
}
...
func CreateGRPCWireRemoteTriggered(wireDef *pb.WireDef, stopC chan struct{}) (*GRPCWire, error) {
	...
	if ok && grpcWire.IsReady {
		grpcLogger.Infof("[ADD-WIRE:REMOTE-END] This grpc-wire is already created by %s. Local interface id : %d peer interface id : %d", grpcWire.Originator, grpcWire.LocalNodeIfaceID, grpcWire.PeerIfaceID)
		return grpcWire, nil
	}
	...
	if err = koko.MakeVeth(inContainerVeth, hostEndVeth); err != nil {
		grpcLogger.Infof("[ADD-WIRE:REMOTE-END] Error creating vEth pair (in:%s <--> out:%s). Error-> %s", inIfNm, outIfNm, err)
		return nil, err
	}
	locIface, err := net.InterfaceByName(hostEndVeth.LinkName)
	if err != nil {
		grpcLogger.Fatalf("[ADD-WIRE:REMOTE-END] Could not get interface index for %s. error:%v", hostEndVeth.LinkName, err)
		return nil, err
	}
	grpcLogger.Infof("[ADD-WIRE:REMOTE-END] Trigger from %s:%d : Successfully created pod to node vEth pair %s@%s <--> %s(%d).", wireDef.PeerIp, wireDef.PeerIntfId, inIfNm, wireDef.LocalPodName, outIfNm, locIface.Index)
	aWire := &GRPCWire{
	...
	*/
	...
	if err != nil {
		grpcLogger.Fatalf("[ADD-WIRE:REMOTE-END] Could not open interface for sed/recv packets for containers. error:%v", err)
		return nil, err
	}
...
func RecvFrmLocalPodThread(wire *GRPCWire) error {
	...
	*/
	...
	if err != nil {
		grpcLogger.Fatalf("Receive Thread for local pod failed to open interface: %s, PCAP ERROR: %v", wire.LocalNodeIfaceName, err)
		return err
	}
	defer rdHandl.Close()
	...
	if err != nil {
		grpcLogger.Fatalf("Receive Thread for local pod failed to set up capture direction: %s, PCAP ERROR: %v", wire.LocalNodeIfaceName, err)
		return err
	}

	remote, err := grpc.Dial(url, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		grpcLogger.Infof("RecvFrmLocalPodThread:Failed to connect to remote %s", url)
		return err
	}
	defer remote.Close()
	...
	wireClient := pb.NewWireProtocolClient(remote)
	...
	for {
		select {
		case <-wire.StopC:
			grpcLogger.Infof("RecvFrmLocalPodThread: closing connection with remote peer-iface@peer-node-ip: %d@%s", wire.PeerIfaceID, wire.PeerNodeIP)
			return io.EOF
		case packet = <-in:
			data := packet.Data()
			payload := &pb.Packet{
			...
			if len(data) > 1518 {
				...
				grpcLogger.Infof("RecvFrmLocalPodThread: unusually large packet received from local pod (may be GRO enabled). size: %d, pkt:%s", len(data), pktType)
			...
			if err != nil || !ok.Response {
				grpcLogger.Infof("RecvFrmLocalPodThread: Could not deliver pkt. Peer not ready. err=%v", err)
